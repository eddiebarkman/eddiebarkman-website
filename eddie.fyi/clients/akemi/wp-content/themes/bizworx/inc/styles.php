...
function bizworx_custom_styles($custom) {
	...
	if ( $body_fonts !='' ) {
		$custom .= "body, #site-navigation ul ul a { font-family:" . $body_fonts . "!important;}"."\n";
	}
	if ( $headings_fonts !='' ) {
		$custom .= "h1, h2, h3, h4, h5, h6, #site-navigation ul li a, .portfolio-info, .roll-testimonials .name, .type-team .team-content .name, .type-team .team-thumb .team-pop .name, .roll-tabs .menu-tab li a, .roll-testimonials .name, .roll-project .project-filter li a, #secondary .widget_recent_entries li a, .banner-button, button, .woocommerce .cart_totals .button, input[type=\"button\"], input[type=\"reset\"], input[type=\"submit\"] { font-family:" . $headings_fonts . ";}"."\n";
	}
    ...
    if ($menu_size) {
        $custom .= "#site-navigation ul li a { font-size:" . intval($menu_size) . "px; }"."\n";
		$custom .= ".nav-cart .cart-count { top:calc(-" . intval($menu_size) . "px - 5px); }"."\n";
    }    	    	
	//H1 size
    ...
    if ($banner_title_size) {
        $custom .= ".header-content .maintitle, .slider-content .maintitle { font-size:" . intval( $banner_title_size ) . "px; }"."\n";
    }
    $banner_subtitle_size = get_theme_mod( 'banner_subtitle_size', '18' );
    if ($banner_subtitle_size) {
        $custom .= ".header-content .subtitle, .slider-content .subtitle { font-size:" . intval( $banner_subtitle_size ) . "px; }"."\n";
    }
	...
	$custom .= "#site-navigation .sub-menu li a { color:" . esc_attr($submenu_items_color) . "}"."\n";
	//Sub menu background
	$submenu_background = get_theme_mod( 'submenu_background', '#1c1c1c' );
	$custom .= "#site-navigation .sub-menu li a { background:" . esc_attr($submenu_background) . "}"."\n";
	...
	//$custom .= ".owl-wrapper { height: " . esc_attr($header_bg_height) . "px; overflow: hidden; }"."\n";
	
	//Page wrapper padding
	$pg_top_padding = get_theme_mod( 'wrapper_top_padding', '83' );
	$pg_bottom_padding = get_theme_mod( 'wrapper_bottom_padding', '100' );
	$custom .= ".page-wrap { padding-top:" . intval($pg_top_padding) . "px;}"."\n";	
